    ...
    private Bitmap getBitmapFromDrawable(Drawable drawable) {
        ...
        try {
            ...
            if (drawable instanceof ColorDrawable) {
                ...
                if (width <= 0 || height <= 0) {
                    int imgWidth = (int) getTag(R.id.chat_list_image_view_width);
                    int imgHeight = (int) getTag(R.id.chat_list_image_view_height);
                    if (imgWidth <= 0 || imgHeight <= 0) {
                        bitmap = Bitmap.createBitmap(ChatConstant.DEFAULT_IMAGEVIEW_WIDTH, ChatConstant.DEFAULT_IMAGEVIEW_HEIGHT, BITMAP_CONFIG);
                    } else {
                        bitmap = Bitmap.createBitmap(imgWidth, imgHeight, BITMAP_CONFIG);
                    }
                } else {
