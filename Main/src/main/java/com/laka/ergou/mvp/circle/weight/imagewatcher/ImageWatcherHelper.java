...
public class ImageWatcherHelper {
    ...
    public void show(ImageView imageView, SparseArray<ImageView> imageGroupList, List<Uri> urlList, WechatShareBean item,int i) {
        mPostion = i;
        mWechatShareBean = item;
        init();
        final boolean displaySuccess = mImageWatcher.show(imageView, imageGroupList, urlList);
        if (displaySuccess) {
            appendOtherView();
        }
    }
    ...
    private void init() {
        ...
        mImageWatcher.setWechatShareBean(mWechatShareBean);
        mImageWatcher.setPostion(mPostion);
