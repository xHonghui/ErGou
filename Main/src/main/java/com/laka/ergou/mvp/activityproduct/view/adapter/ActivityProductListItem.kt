 ...
 * @Description:商品列表单列
 */
class ActivityProductListItem : MultipleAdapterItem<ProductWithCoupon> {
    ...
    override fun convert(helper: BaseViewHolder?, item: ProductWithCoupon?) {
        ...
        item?.let {
            ...
            tvProductPrice?.text = BigDecimalUtils.numberFormat(item.actualPrice)
            TextViewHelper.setSpan(TextViewHelper.Builder()
                    ...
                    .setStrike(true))
            tvProductCommission?.text = ResourceUtils.getStringWithArgs(R.string.normal_product_commission, BigDecimalUtils.numberFormat("${item?.commissionPrice}"))
            ...
            when (item.sellerType) {
                ActivityProductConstant.TYPE_USER_TMALL -> { //天猫
                    ivIcon?.setImageResource(R.drawable.label_icon_taobao)
                }
                ActivityProductConstant.TYPE_USER_TAOBAO -> {  //淘宝
