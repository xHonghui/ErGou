package com.laka.ergou.mvp.circle.weight.imagewatcher;
...
import com.laka.ergou.mvp.circle.model.bean.Product;
...
public class ImageWatcher extends FrameLayout implements GestureDetector.OnGestureListener, ViewPager.OnPageChangeListener {
    ...
    protected int mErrorImageRes = R.drawable.default_img; // 图片加载失败站位图
    ...
    public void setWechatShareBean(WechatShareBean item) {
        this.mWechatShareBean = item;
    }
...
//        float distance = (float) Math.sqrt(deltaX * deltaX + deltaY * deltaY);
                ...
                loader.load(imageView.getContext(), mUrlList.get(position), new LoadCallback() {
                    ...
                    public void onResourceReady(Drawable resource) {
                        if (position == mPostion) {
                            resource = resetResource(resource);
                        }
    ...
    private Drawable resetResource(Drawable resource) {
        if (mWechatShareBean == null) return resource;
        View view = LayoutInflater.from(getContext()).inflate(R.layout.activity_see_share_image, null, false);
        TextView tvShopTitle = view.findViewById(R.id.tv_shop_title);
        TextView tvShopPrice = view.findViewById(R.id.tv_shop_price);
        TextView tvCouponValue = view.findViewById(R.id.tv_coupon_value);
        TextView tvOriginPrice = view.findViewById(R.id.tv_origin_price);
        TextView tvVolume = view.findViewById(R.id.tv_volume);
        ImageView imageView = view.findViewById(R.id.iv_shop_detail);
        ImageView ivQrCode = view.findViewById(R.id.iv_qr_code);
        tvShopTitle.setText(mWechatShareBean.getTitle());
        tvShopPrice.setText(mWechatShareBean.getActualPrice());
        tvCouponValue.setText(mWechatShareBean.getCouponMoney() + "");
        tvOriginPrice.setText("￥" + mWechatShareBean.getZkFinalPrice());
        tvVolume.setText("销量" + mWechatShareBean.getVolume());
        Bitmap bitmap = QRCodeUtil.createQRCodeBitmap(mWechatShareBean.getTklShareUrl(), ScreenUtils.dp2px(83f), 0);
        if (bitmap != null) {
        ...
        imageView.setImageDrawable(resource);
        Bitmap bitmap1 = createBitmap3(view, ScreenUtils.getScreenWidth(), ScreenUtils.getScreenHeight());
        resource = new BitmapDrawable(bitmap1);
        return resource;
    }
